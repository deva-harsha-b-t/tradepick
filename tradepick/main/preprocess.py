...
class config:
    ...
    #     ticker = fin.read()
    #     # ticker = guli.GuliVariable("hello").get()
    # # print("Performing analysis for the Ticker : ", ticker)
    # stock_names_compare = [ticker]
    # stock_names = [ticker]
...
def plot_gain(df):
    ...
    df_copy = df[['Open', 'Close']]

    df_diff = df_copy.diff(axis=1)

    df['Daily Return'] = df_diff[['Close']]

    df['Change %'] = df["Daily Return"].div(df["Open"].values)
