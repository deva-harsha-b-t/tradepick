...
def get_data(csv):
    if csv:
        ...
        timestamps = get_timestamps(config.yrs,
                                    config.mths,
                                    config.dys)
...
def data_preparation(df):
    ...
    df_new = df.copy()
    df_new["Actual"] = df_new["Mean"].shift(
        analysis_config.lookback_days).dropna()
...
def analyse_movement(df):
    ...
    seas_d = sm.tsa.seasonal_decompose(
        df["Mean"], model="add", period=analysis_config.season_period)
    ...
    dates = [str(seas_d.trend.index.values[i]).split('T')[0]
             for i in range(len(seas_d.trend.index.values))]
    ...
    [first_plot_labels_new.append(dates[i * nr_label_indexes])
     for i in range(len(first_plot_labels))]
    ...
    second_plot_labels = [item.get_text()
                          for item in axes[1].get_xticklabels()]
    nr_label_indexes = len(dates) // (len(second_plot_labels)-2)
    second_plot_labels_new = [str(dates[0])]
    [second_plot_labels_new.append(dates[i * nr_label_indexes])
     for i in range(len(second_plot_labels)-2)]
    ...
    [third_plot_labels_new.append(dates[i * nr_label_indexes])
     for i in range(len(third_plot_labels))]
    ...
    fourth_plot_labels = [item.get_text()
                          for item in axes[3].get_xticklabels()]
    nr_label_indexes = len(dates) // (len(fourth_plot_labels)-2)
    fourth_plot_labels_new = [str(dates[0])]
    [fourth_plot_labels_new.append(dates[i * nr_label_indexes])
     for i in range(len(fourth_plot_labels)-2)]
...
def ADFtest(time_series):
    ...
    dfout = {}
    dftest = sm.tsa.adfuller(time_series.dropna(),
                             autolag='AIC', regression='ct')
    for key, val in dftest[4].items():
        dfout[f'critical value ({key})'] = val
    if dftest[1] <= 0.05:
        print(
            f"Strong evidence against Null Hypothesis, p-value: {dftest[1]:.4f} < 0.05")
        # print("Reject Null Hypothesis - Data is Stationary")
        return True
    else:
        print("Strong evidence for Null Hypothesis")
        print(
            f"Accept Null Hypothesis - Data is not Stationary, p-value: {dftest[1]:.4f} > 0.05")
        return False
...
def get_stationary_data(df: pd.DataFrame, columns: list, diff: int):
    ...
    df_cp = df.copy()
    for col in columns:
        df_cp[str(col)] = pd.DataFrame(
            np.log(df_cp[str(col)]).diff().diff(diff))
    return df_cp
...
def plot_stationary(df, df_stat):
    ...
    dates = [str(df.index.values[i]).split('T')[0]
    ...
    [plot_labels_new.append(dates[i * nr_label_indexes])
